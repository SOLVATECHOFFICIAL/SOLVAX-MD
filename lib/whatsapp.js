...
function createPairingState(
    ...
    /*
     * If an old pairing state exists,
     * clear its timeout first.
     */
    const previous =
...
async function createWhatsAppSession(
    ...
    try {
        ...
        if (
            ...
            /*
             * Cancel old pairing timer/state.
             */
            const oldPairing =
                global.pairingStates[id];
            ...
            setTimeout(
                async () => {
                    ...
                    try {
                        ...
                        console.error(
                            `[WhatsApp] Pairing timeout cleanup failed for ${id}:`,
                            error
                        );
                    }
...
async function handleIncomingMessages(
    ...
    for (
        ...
        try {
             ...
             *
             * This prevents many simple bot loops.
             */
            if (
                message.key &&
                message.key.fromMe
            ...
            /*
             * Dispatch to the project's message handler.
             */
 ...
 * ============================================================
 *
 * We deliberately do not hard-code one handler filename.
 *
 * Supported:
 *
 * lib/whatsapp-handler.js
 * lib/whatsappHandler.js
 * lib/messageHandler.js
 * root/whatsapp-handler.js
 *
 * ============================================================
 */
...
async function dispatchWhatsAppMessage(
    ...
    for (
        ...
        try {
            ...
            console.error(
                `[WhatsApp] Handler error from ${handlerPath}:`,
                error
            );
     ...
     * No handler found.
     *
     * This is not a fatal WhatsApp connection error.
     */
    return;
}
 ...
 * ============================================================
 *
 * Commands from the same WhatsApp account are executed in
 ...
 *
 * This matters if multiple files require whatsapp.js.
 */
if (

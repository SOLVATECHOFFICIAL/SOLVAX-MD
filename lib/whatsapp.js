...
function hasLink(text) {
    return /(?:https?:\/\/|www\.|\b[a-z0-9-]+\.(?:com|net|org|io|co|ng|uk|me|xyz)\b)/i.test(
        String(text || '')
    );
}
...
async function enforceGroupSettings(
    ...
    /*
     * Anti-link is enforced for non-admin members only
     * when the bot can delete messages.
     */
    ...
    /*
     * When muted, only the linked account and group admins
     * may invoke commands.
     */
 ...
 * IMPORTANT FIX:
 *
 * lib/queue.js defines:
 *
 *     enqueueCommand(task, key)
 *
 * The task MUST be the first argument.
 *
 * ============================================================
 */

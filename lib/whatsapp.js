...
async function handleIncomingMessages(id,sock,update){const session=getWhatsAppSession(id);if(!session||session.socket!==sock)return;for(const msg of update?.messages||[]){try{const jid=getRemoteJid(msg);if(!jid||isIgnoredJid(jid)||!msg.message)continue;const text=extractMessageText(msg);if(await enforceGroupSettings(id,session,msg,jid,text))continue;if(!isSelfMessage(msg))continue;if(!text)continue;await dispatchWhatsAppMessage(id,msg);}catch(e){console.error(`[WhatsApp] message processing ${id}:`,e?.stack||e);}}}
